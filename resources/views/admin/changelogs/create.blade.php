...
<div class="container mx-auto p-6">
    ...
    <form action="{{ route('admin.changelogs.store') }}" method="POST" class="space-y-4 bg-white p-6 shadow rounded">
        ...
        <div>
            <label class="block font-semibold">リンク（任意）</label>
            <input type="url" name="link" value="{{ old('link') }}"
                placeholder="例: https://kasumizaka46.com/members/stats"
                class="border p-2 rounded w-full">
            <p class="text-xs text-gray-500 mt-1">入力するとトップの更新履歴タイトルがリンクになります。</p>
        </div>
